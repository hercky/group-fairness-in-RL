...
from deepRL_algos.ppo import PPO
...
if __name__ == "__main__":
    ...
    for z in range(num_subgroups):
        ...
        # Setup FCPO agent
        ppo.append(PPO(args=args, subgroup=z, agent=Agent(envs[z]).to(device)))
    ...
    def update_test_results(step):
        ...
        for i in range(num_test_envs):
            ...
            for z in range(num_subgroups):
                test_returns[z] = sample_return(ppo[z].agent, test_envs[z][i], device)
            # sampled once
    ...
    #           Training Loop
    # ------------------------------------------------------- #
    global_step = 0  # Note: what does this even sigify now?
    ...
    for z in range(num_subgroups):
        rollouts.append(data_gen[z].collect_data_and_targets(ppo[z].agent, envs[z], writer))
    ...
    for update in range(1, num_updates + 1):
        ...
        if args.anneal_lr:
            ...
            # do for both subgroups
            for z in range(num_subgroups):
                ppo[z].optimizer.param_groups[0]["lr"] = lrnow
        ...
        assert data_gen[0].global_step == data_gen[
            1].global_step, "Unequal amount of samples for both subgroups"
        ...
        # ---------- Update the agents --------------
        for z in range(num_subgroups):
            ppo[z].update_params(rollouts[z], data_gen[z].global_step, writer)
            # Update the rollouts
            rollouts[z] = data_gen[z].collect_data_and_targets(ppo[z].agent, envs[z], writer)
            writer.add_scalar(f"charts/{z}/SPS", int(data_gen[z].global_step / (time.time() - start_time)),
                              data_gen[z].global_step)
