...
class MazeCell(Enum):
    ...
    XY_HALF_BLOCK = 18
    SPIN = 19
    # [h] Additional components for populations
    PHANTOM_BLOCK = 101
    ...
    def is_phantom_block(self) -> bool:
        # return self == self.PHANTOM_BLOCK
        return self == self.PHANTOM_BLOCK
...
class CollisionDetector:
    ...
    def __init__(
        ...
        for i, j in it.product(range(len(structure)), range(len(structure[0]))):
            # skip if the structure is not a block
            # if not structure[i][j].is_block():
            #     continue
            # treat phantom block as a block for collision
            # skip if the structure is neither a block or a phantom block
            if not (structure[i][j].is_phantom_block() or structure[i][j].is_block()):
                continue
